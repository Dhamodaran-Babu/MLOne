def Auto_Fit(filepath,bias_var=False):
    ...
    import warnings
    from MLOne.Validation import validate_data

    warnings.filterwarnings("ignore")
    data,status = validate_data(filepath)
    if status :
        from MLOne.eda import explore_data
        explore_data(data=data)
        
        from MLOne.preprocessing import preprocessor
        x,y,pbm_type = preprocessor(data)
        
        from MLOne.splitter import train_test_validation_splitter
        ...
        from MLOne.classification_models import model_fitter
        ...
        if bias_var:
            from MLOne.bias_variance import bv_decomp
            bv_decomp(all_estimators,xtrain,ytrain.flatten(),xval,yval.flatten())

        from MLOne.interpret_results import interpret_results
